...
const PRESET_RUNTIME_SECTIONS = new Set(['orbitSweep', 'waves']);
const ALWAYS_STICKY_SECTIONS = new Set(['waves']);
...
function sectionPaths(sectionKey) {
  const fields = schema[sectionKey]?.fields || {};
  return Object.keys(fields).map((fieldKey) => `${sectionKey}.${fieldKey}`);
}

const alwaysStickyPaths = new Set([
  ...[...ALWAYS_STICKY_SECTIONS].flatMap(sectionPaths),
]);
...
for (const p of alwaysStickyPaths) {
  if (stickyMap[p] === undefined) stickyMap[p] = structuredClone(getAt(defaultParams, p));
}
const stickyPaths = new Set([...Object.keys(stickyMap), ...alwaysStickyPaths]);
...
if (bootPreset && bootPreset.params) {
  ...
  for (const p of alwaysStickyPaths) {
    if (stickyMap[p] !== undefined) setAt(boot, p, stickyMap[p]);
  }
} else {
...
function handleAction(action) {
  switch (action) {
    case 'default':
      ...
      for (const p of alwaysStickyPaths) {
        const v = structuredClone(getAt(defaultParams, p));
        stickyPaths.add(p);
        stickyMap[p] = v;
        setSticky(p, v, true);
      }
...
// H/B panel · C water · G god rays · X shadows · F fps · R randomize · Esc closes panel (NOT D — D is WASD).
